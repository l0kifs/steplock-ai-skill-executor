...
A helper script is a utility the agent can call **during a step** to gather information or perform preparatory work before submitting output. Helpers are declared per-step with the `helpers` list:
...
The agent invokes them via the `run_helper_script` MCP tool (see below). Each helper is identified by its **filename without extension** (e.g. `get_context` for `scripts/get_context.py`). Helper script names must be unique within a step.

Helpers receive optional string arguments via `sys.argv` and write their results to **stdout**. The tool returns `stdout`, `stderr`, and `exit_code`.
...
StepLock exposes four tools to the connected agent.
...
```json
{
  ...
  "helper_scripts": ["get_context", "list_files"]
}
```

`helper_scripts` lists the names of helpers available on the current step. Empty list if none are declared.
...
```json
{ "status": "next_step", "step_id": "step-2", "instruction": "...", "helper_scripts": [] }
{ "status": "completed" }
{ "status": "retry",   "step_id": "step-1", "instruction": "...", "message": "guidance from verify script", "helper_scripts": ["get_context"] }
{ "status": "aborted", "message": "guidance from verify script" }
```

`helper_scripts` is included in `next_step` and `retry` responses, listing helpers available on the returned step.

### `run_helper_script`

Runs a helper script declared on the current step. Call this any time during a step before submitting output.

| Parameter | Type | Description |
|-----------|------|-------------|
| `session_id` | `string` | Session identifier from `start_skill` |
| `script_name` | `string` | Helper name as listed in `helper_scripts` |
| `args` | `string[]` | Optional arguments passed to the script via `sys.argv` |
...
| `steps[].helpers` | No | `[]` | List of helper script paths (relative to skill dir) |
...
Save the file, accept the trust dialog, and the four tools (`list_skills`, `start_skill`, `submit_step_output`, `run_helper_script`) become available in Copilot Chat.
